...
function installAuthUpgrade(app, services) {
  ...
  const demoStudentBtn = replaceButton(document.getElementById("demoStudentBtn"), () => activateDemo("student"));
  const demoTeacherBtn = replaceButton(document.getElementById("demoTeacherBtn"), () => activateDemo("teacher"));
  ...
  function activateDemo(role) {
    const demoProfile = {
      ...
      stats: {
        xp: role === "teacher" ? 220 : 140,
        level: role === "teacher" ? 2 : 2
      },
      badges: role === "teacher" ? ["Mentor Mark"] : ["First Spark"]
    };
    app.state.demoMode = true;
    app.applyAuthenticatedProfile(demoProfile.uid, profileForLegacy(demoProfile));
    app.showToast(`${demoProfile.name} loaded in demo mode`);
  }
